class Validator {
    ...
    switch (fieldName) {
      ...
      case 'login':
        if (/^(?=.*[a-zA-Z])[a-zA-Z0-9_-]{3,20}$/.test(value)) {
          ...
          return { isValid: false, error: 'Логин должен быть от 3 до 20 символов, латиница, может содержать цифры, дефисы и подчеркивания.' };
        }
      ...
      case 'old_password':
        if (/^(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{8,40}$/.test(value)) {
          ...
          return { isValid: false, error: 'Пароль должен быть от 8 до 40 символов, содержать хотя бы одну заглавную букву и цифру.' };
        }
      ...
      case 'repeat_password':
        const passwordInput = this.findInputInForm(field, 'password');
        if (passwordInput) {
  ...
  static findInputInForm(currentInput: HTMLInputElement, targetInputName: string): HTMLInputElement | null {
    ...
    let parentElement: HTMLElement | null = currentInput.parentElement;
    while (parentElement) {
